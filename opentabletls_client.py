...
def reservation():
    session=tls_client.Session(client_identifier="confirmed_android",random_tls_extension_order=True)
    session.headers={
        "path": "/",
        "scheme": "https",
    ...
    response=session.execute_request(method="POST",url=f"https://www.opentable.com/dapi/dag/v1/single/initcheckout",json=payload1)
    # print(vars(session))
    payload = {
    "additionalServiceFees": [],
    "attributionToken": f"x={formatted_time}&c=1&pt1=1&pt2=1&er=1268701",
    ...
    # print(session._session_id)
    # response=session.execute_request(method="GET",url=f"https://www.opentable.com/s?dateTime={formatted_time}&covers=2&ridsAddons%5B%5D=1268701&latitude=40.7685526&longitude=-73.9831866&shouldUseLatLongSearch=true",data=payload)
    response=session.execute_request(method="GET",url=f"https://www.opentable.com/r/bad-roman-new-york?corrid={uuidcorrelationid}&avt=eyJ2IjoyLCJtIjowLCJwIjowLCJzIjowLCJuIjowfQ&p=2&sd=2024-03-03T19%3A00%3A00",json=payload)
    response=session.execute_request(method="POST",url=f"https://www.opentable.com/dapi/booking/make-reservation",json=payload)
