...
# print(time.localtime().tm_mday,time.localtime().tm_hour,time.localtime().tm_min,time.localtime().tm_mday,)
jour="25"
mois="03"
annee="2024"
heure="22"
...
if os.path.exists("./cache/cookies.pickle") and os.path.exists("./cache/headers.json"):
    ...
    sessionlogin=mainV2.login(session=session)
    ...
    # with open("./cache/headers.json","wb") as f:
    #     json.dump(dict(sessionlogin.headers), f,indent=4)
    # if os.path.exists("./cache/header.txt")

# session.headers = sessionlogin.headers
# session.cookies= sessionlogin.cookies

# session.get("https://www.opentable.fr/")
